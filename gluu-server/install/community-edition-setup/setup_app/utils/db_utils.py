...
class DBUtils:
    ...
    def read_gluu_schema(self, others=[]):
        self.gluu_attributes = []

        for schema_fn_ in ['gluu_schema.json', 'custom_schema.json'] + others:
            ...
            self.gluu_attributes += schema['attributeTypes']
    ...
    def set_oxAuthConfDynamic(self, entries):
        ...
        if self.moddb == BackendTypes.LDAP:
            ...
            sqlalchemyObj.oxAuthConfDynamic = json.dumps(oxAuthConfDynamic, indent=2)
            self.session.commit()
            ...
            self.spanner.update_data(table='oxAuthConfiguration', columns=['doc_id', 'oxAuthConfDynamic'], values=[[doc_id, json.dumps(oxAuthConfDynamic)]])
    ...
    def enable_service(self, service):
        if self.moddb == BackendTypes.LDAP:
            ...
            sqlalchemyObj = self.get_sqlalchObj_for_dn('ou=configuration,o=gluu')
    ...
    def add_client2script(self, script_inum, client_id):
        ...
        if backend_location == BackendTypes.LDAP:
            ...
            sqlalchemyObj = self.get_sqlalchObj_for_dn(dn)
            if sqlalchemyObj:
                if sqlalchemyObj.oxConfigurationProperty:
                    oxConfigurationProperty = copy.deepcopy(sqlalchemyObj.oxConfigurationProperty)
                else:
            ...
            data = self.spanner.exec_sql('SELECT oxConfigurationProperty from oxCustomScript WHERE dn="{}"'.format(dn))
            oxConfigurationProperty = []
            added = False
            spanner_data = self.spanner_to_dict(data)
            oxConfigurationProperty = spanner_data.get('oxConfigurationProperty', [])

            for i, oxconfigprop in enumerate(oxConfigurationProperty[:]):
                ...
                if oxconfigpropjs.get('value1') == 'allowed_clients':
                    ...
                    oxConfigurationProperty[i] = json.dumps(oxconfigpropjs)
                    added = True
                    break

            if not added:
                oxConfigurationProperty.append(json.dumps({'value1': 'allowed_clients', 'value2': client_id}))
            self.spanner.update_data(table='oxCustomScript', columns=['doc_id', 'oxConfigurationProperty'], values=[[script_inum,  oxConfigurationProperty]])
    ...
    def get_attr_syntax(self, attrname):
        for gluu_attr in self.gluu_attributes:
            if attrname in gluu_attr['names']:
                if gluu_attr.get('multivalued'):
                    return 'JSON'
                return gluu_attr['syntax']
        else:
    ...
    def import_ldif(self, ldif_files, bucket=None, force=None):
        ...
        sql_data_fn = os.path.join(Config.outputFolder, Config.rdbm_type, 'gluu_data.sql')

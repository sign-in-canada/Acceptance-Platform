...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, customScript, configurationAttributes):
        ...
        if len(self.providers) > 1:
            uiPathParam = configurationAttributes.get("ui_path")
            if uiPathParam is None:
                print ("%s: UI path is missing from config!"  % self.name)
                return False
            else:
                self.uiPath = uiPathParam.getValue2()
        ...
        self.account.init(configurationAttributes, self.name, self.passport)
    ...
    def getExtraParametersForStep(self, configurationAttributes, step):
        return Arrays.asList("stepCount",   # Used to complete the workflow
                             ...
                             "resultId")    # Content identifier for the results step ("results of what task")
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if step in {self.STEP_CHOOSER, self.STEP_MIGRATE, self.STEP_COLLECT_PROMPT, self.STEP_RESULT}:
            ...
            externalPath = "%s%s?lang=%s&id=%s" % (self.uiPath, page, uiLocales.split('-')[0], rpConfig.get("content"))
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        elif requestParameters.containsKey("navigate:continue"):
